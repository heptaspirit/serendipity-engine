...
| [`docs/api-contract.md`](docs/api-contract.md) | API 契约：12 端点 + 鉴权（插件仓库与引擎的唯一共享物，改 API 必同步） |
