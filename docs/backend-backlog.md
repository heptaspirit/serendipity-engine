...
功能像开关一样开合：选库 → 漫游 / 详情 / 随机 / 导出 / 刷新触发；serve·MCP 是长驻服务、
已有自己入口，**不进 TUI 主流程**，只作为"可开关的服务"管理（启动/停止/显示 URL/token，守护进程控制台式）。
现有子命令（roam/serve/index/mcp…）**全部保留**（脚本/agent/--json 通道不变），TUI 是无参数时的默认入口。
