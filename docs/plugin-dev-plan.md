...
> **〔2026-08-26 暂停声明〕虎鲸版本插件不开发**。尝试后放弃：虎鲸生态小、插件壳收益低；且**内核已直读虎鲸库**（`seren index/roam/serve <库.db>` 照常可用）——虎鲸用户直接用内核即可完成同等功能，无需插件。M2 插件薄壳收敛为 **Obsidian 单壳**；本文虎鲸相关小节均已注释/标注暂停，保留作调研溯源（完整调研见 [`plugin-evaluation.md`](history/plugin-evaluation.md)）。
...
| D4 | 独立仓库，零构建时依赖（`serendipity-engine` / `serendipity-obsidian`；~~`serendipity-orca` 暂停~~） | 引擎/插件只运行时契约引用，不 submodule/vendor | plugin-evaluation §五 |
...
| Q4 | 四态机 + coreManagement（Obsidian=managed；~~虎鲸=external 暂停~~） | Obsidian 可 spawn（managed） | §六 |
...
<!-- 〔2026-08-26 暂停〕虎鲸版本插件不开发，本节注释保留作调研溯源。内核直读虎鲸库能力不受影响。
...
> **〔2026-08-26 暂停〕虎鲸版本插件不开发**（本节调研内容已注释保留，见上）。

### 3.3 开发难度评估（跨平台对比，深读结论）

> 虎鲸列已暂停（2026-08-26），保留作对比参考；下表仅 **Obsidian 列**为有效开发目标。

| 维度 | Obsidian 插件 | ~~虎鲸插件（暂停）~~ |
|---|---|---|
...
- ~~**虎鲸难度低于此前预期**：面板/导航/命令/斜杠/工具栏/快捷键/状态订阅一应俱全，薄壳几乎是「注册面板 + iframe + 探测 + 跳回」四步；主要限制是 external 进程（用户自跑核心）与手动分发（无市场）。~~（〔2026-08-26 暂停〕）
- **Obsidian 难度最高**（进程管理 + 审核 + 侧栏视图），但能力最自然，应先做。
- **先做 Obsidian**（~~虎鲸次之~~ 已暂停，2026-08-26）。
...
- **Q2 核心分发**：
  ...
  - **v1.x 增强**：设置内「下载核心」按钮——取 latest release → 按平台选 asset → 下载落盘。Obsidian 端用 `fs`（~~虎鲸端用 `plugins.writeFile`~~ 已暂停）均技术可行；唯「启动核心」仍靠用户（external 物理边界不可破）。
  - ~~external 模式（虎鲸）只需 host/port，不需 corePath。~~（〔2026-08-26 暂停〕）

---

## 五、仓库结构与依赖（Q3）

- **两个独立仓库**：`serendipity-engine`（Go 核心 + Web UI + release 二进制）/ `serendipity-obsidian`（~~`serendipity-orca` 已暂停~~）；**零构建时依赖**（不 submodule、不 vendor）。
...
- **coreManagement 双模式**：
  ...
  - ~~**虎鲸 = external**：用户自跑核心（终端 / launchd / 系统服务），插件只连；`load` 探活、不通则显示「未检测到引擎」+ 下载链接 + 启动命令引导。~~（〔2026-08-26 暂停〕）
...
- ~~**M2-2 后虎鲸**：external 纯 glue，复用同一 Web UI 与 REST 契约。~~（〔2026-08-26 暂停〕）
- **预期**：插件工作约 **80% 胶水 / 20% 平台特化**（~~两端~~单端共享引擎核心 + Web UI + 契约）。
...
- ~~**虎鲸**（详见 plugin-evaluation §九 9.7）：`broadcasts` 的 `type` 字符串集、设置 schema 类型集、PanelProps 精确字段、`invokeBackend` 支持的 `type`、`shortcuts.assign` 命令格式。~~（〔2026-08-26 暂停〕）
...
- **AIBackend 抽象**：Obsidian=本地/云 HTTP（用户配 endpoint+key）；~~虎鲸=`orca.ai` 桥接~~（〔2026-08-26 暂停〕）。AI 只看"笔记内容 + 引擎事实"，绝不生成笔记内容进图。
...
<!-- 〔2026-08-26 暂停〕虎鲸版本插件不开发，链接保留作调研溯源。
