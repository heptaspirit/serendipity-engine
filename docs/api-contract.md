...
| v0.2.2 | **adapter/frontmatter**：frontmatter 改由 **yaml.v3** 解码（替换手写 mini-YAML）——支持块标量（`\|`/`>` 多行值）、引号转义、嵌套缩进列表；键字符集与标量原文口径与旧版一致；解析器版本 bump → 升级后旧 store 自动**全量重析**（parser_version 失效机制，不混新旧解析结果）。**引擎**：MCP SDK 升 mcp-go **v1.0.0**（首个正式版）；错误方法请求由 200+JSON 改回标准 405（A/B 档 ServeMux 方法模式）；内部重构（ponytail 审计 A/B/C/D，行为等价，零 API 面变化）。 |
