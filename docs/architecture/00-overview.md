...
| v0.1.14 | **touch 行为信号子系统（backlog §3.7）**：touch 拆独立 store `touch-<hash>.bbolt`（touch/meta/backups 三 bucket，图库重建不再连坐）；digest 触发（计数 ≥ `digest_count` 主 / 间隔 ≥ `digest_days` 兜底 + serve 启动补查）+ `GET /api/touch/digest`（只读）+ `POST /api/touch/digest/ack`（已读）+ `/api/stats.digest_available` + MCP `seren.touch_digest`（只读八工具）；`touch.yaml` 参数（YAML 钳制）；**引擎零写 vault**（`serendipity-digest-*.md` 由前端插件导出，§3.7.3） |
| v0.1.15 | **无库启动 + 配库（壳/TUI/Wails 的地基）**：`seren serve` 无 vault 空库启动，`POST /api/vault` 配库/换库（换图 + 闭包重建 + watch 重启，幂等）+ `GET /api/vault` 查配置；`/api/stats` 加 `configured`；web 路由全量注册 + handler 内闭包 nil 判定；前端未配库显示选库引导 + 配库后显示已配库状态条；当前节点操作栏（详情/相似/关系一键）+ 别名显示 + 预览分组；优雅退出（SIGINT/SIGTERM → 停 watch → Shutdown）；终端 URL OSC 8 可点击链接；前端导出改 fetch+blob（a 标签不带 token 被 auth 拒的修复） |
